...
class DeepLabMultiInput(nn.Module):
    ...
    def forward(self, input1, input2=None, input3=None, input4=None, mask=None):
        ...
        # print(f"Input Image Shape: {input1.shape}")   # batch x 3 x 1024 x 1024
        x1, low_level_feat1 = self.backbone1(input1)
        # print(f"Resnet Output Shape: {x1.shape}, low level feature shape: {low_level_feat1.shape}")   # x = batch x 2048 x 64 x 64, llf = batch x 256 x 256 x 256
        x1 = self.aspp1(x1)
        ...
        if input3 is not None:
            # print(input3.shape)     # batch x 1 x 1024 x 1024
        ...
        if input4 is not None:
            # print(input4.shape)     # batch x 1 x 1024 x 1024
        ...
        # print(f"High Level Feature Shape: {x.shape}")    # batch x 1024 x 64 x 64

        low_level_feat = torch.cat([low_level_feat1,low_level_feat2,low_level_feat3,low_level_feat4], dim=1)
        # print(f"Low Level Feature Shape: {low_level_feat.shape}")    # batch x 1024 x 256 x 256
        x = self.decoder(x, low_level_feat, mask)
        # print(f"Decoder Output Shape: {x.shape}")     # batch x 20 x 256 x 256

        x = F.interpolate(x, size=input1.size()[2:], mode='bilinear', align_corners=True)
        # print(f"Interpolation Output Shape: {x.shape}")    # batch x 20 x 1024 x 1024
