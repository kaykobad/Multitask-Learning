...
class Decoder(nn.Module):
    ...
    def forward(self, x, low_level_feat, mask):
        ...
        # print(f"Low level Feature after Decoder Conv Shape: {low_level_feat.shape}")     # batch x 48 x 256 x 256
        x = F.interpolate(x, size=low_level_feat.size()[2:], mode='bilinear', align_corners=True)
        # print(f"High level Feature after Interpolation Shape: {x.shape}")      # batch x 1024 x 256 x 256
