...
class DeepLabV3Plus(nn.Module):
    def __init__(self, backbone='resnet', output_stride=16, num_classes=20, sync_bn=True, freeze_bn=False, pretrained=True, num_modalities=1):
        ...
        self.backbone1 = build_backbone(backbone, output_stride, BatchNorm, pretrained=True, input_dim=3)  # RGB
        self.aspp1 = build_aspp(backbone, output_stride, BatchNorm)
        self.backbone2 = build_backbone(backbone, output_stride, BatchNorm, input_dim=1)  # NIR
        self.aspp2 = build_aspp(backbone, output_stride, BatchNorm)
        self.backbone3 = build_backbone(backbone, output_stride, BatchNorm, input_dim=2)  # AoLP
        self.aspp3 = build_aspp(backbone, output_stride, BatchNorm)
        self.backbone4 = build_backbone(backbone, output_stride, BatchNorm, input_dim=1)  # DoLP
        self.aspp4 = build_aspp(backbone, output_stride, BatchNorm)
        self.decoder = build_decoder(num_classes, backbone, BatchNorm, self.num_modalities)
    ...
    def forward(self, input1, input2=None, input3=None, input4=None):
        ...
        # print("Num modalities: ", self.num_modalities)

        if input2 is not None:
            # print("Comes inside input2")
