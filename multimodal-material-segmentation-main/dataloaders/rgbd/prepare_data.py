...
from dataloaders.rgbd import preprocessing
from dataloaders.rgbd.nyudv2 import NYUv2


def prepare_data(args, ckpt_dir=None, with_input_orig=True, split=None):
    """
    Gets the dataloader according to the parameters. Vary according to the args param.
    args.dataset: string to choose the dataset used
    args.aug_scale_min: minimum value of rescale (data augmentation)
    args.aug_scale_max: maximum value of rescale (data augmentation)
    args.raw_depth: boolean that defines the depth mode (if True, use raw depth map and not interpolated ones)
    args.dataset_dir: path of the directory containing the dataset
    args.height: height of the images
    args.width: width of the images

    other params available...
    """
