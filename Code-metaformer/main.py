...
def main():
    ...
    if 'convformer' in model or 'caformer' in model:
        create_model_args.update(head_dropout=0.0)

    model = create_model(**create_model_args)

    if num_outputs is None:
        assert hasattr(model, 'num_classes'), 'Model must have `num_classes` attr if not set on cmd line/config.'
        num_classes = model.num_classes  # FIXME handle model default vs config num_classes more elegantly
