...
class LayerNormGeneral(nn.Module):
    ...
    Args:
        ...
        Modified LayerNorm (https://arxiv.org/abs/2111.11418)
            that is identical to partial(torch.nn.GroupNorm, num_groups=1):
...
class SepConv(nn.Module):
    ...
    def __init__(self, dim, expansion_ratio=2, act1_layer=StarReLU, act2_layer=nn.Identity, bias=False, kernel_size=7, padding=3, **kwargs):
        ...
        self.dwconv = nn.Conv2d(med_channels, med_channels, kernel_size=kernel_size, padding=padding, groups=med_channels, bias=bias)  # depthwise conv
...
@register_model
def caformer_s18(pretrained=False, num_classes=1000, **kwargs):
    model = MetaFormer(depths=[3, 3, 9, 3], dims=[64, 128, 320, 512], token_mixers=[SepConv, SepConv, Attention, Attention], head_fn=MlpHead, num_classes=num_classes, **kwargs)
