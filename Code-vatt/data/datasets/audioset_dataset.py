...
class AudiosetFactory(loading.BaseDMVRFactory, abc.ABC):
    """Base class for audioset factory."""
    ...
    def __init__(self, subset='train', split=1):
        """Constructor of AudiosetFactory."""

        del split
        if subset not in AudiosetFactory._TABLES:
            raise ValueError('Invalid subset "{}". The available subsets are: {}'
                             .format(subset, AudiosetFactory._TABLES.keys()))

        super().__init__(AudiosetFactory._BASE_DIR,
                         table=f'{subset}',
                         source='tfrecord',
