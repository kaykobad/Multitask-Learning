...
import configs.audio as audio_config
import configs.experiment as experiment
import configs.multimodal as vatt_config
import configs.text as text_config
import configs.unified as unified_config
import configs.video as video_config
...
def build_experiment_configs(task, model_arch):
    ...
    if task not in EXPERIMENT_CONFIGS:
        raise ValueError('{!r} not found in {!r}'.format(task, EXPERIMENT_CONFIGS.keys()))

    config = EXPERIMENT_CONFIGS[task](model_config=build_model_configs(model_arch))
